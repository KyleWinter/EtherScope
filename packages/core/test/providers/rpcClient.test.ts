...
describe("RpcClient", () => {
  it("supports batchCall and preserves order", async () => {
    const srv = await startRpcServer((body) => {
      // body is an array of json-rpc requests
    ...
    try {
      const rpc = new RpcClient({ url: srv.url, timeoutMs: 2000, retry: { retries: 0, baseDelayMs: 10, jitterMs: 0 } });
  ...
  it("retries on transient failure", async () => {
    const srv = await startRpcServer((body, reqCount) => {
      if (reqCount === 1) {
        // fail first request hard
        throw new Error("boom");
      }
      const reqs = Array.isArray(body) ? body : [body];
      return reqs.map((r: any) => ({ jsonrpc: "2.0", id: r.id, result: "ok" }));
    });
  ...
  it("throws RpcError when JSON-RPC error present", async () => {
    const srv = await startRpcServer((body) => {
